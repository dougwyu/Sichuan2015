...
gfgMB <- tbl_df(gfgMB)
habitat <- read_tsv(habitatinput, col_names = TRUE, na = "NA")
	# cols(
...
habitat_now <- habitat # habitat goes with community_insecta, habitat_diptera goes with community_diptera
...
In the "histogram of otu read numbers" I try removing OTUs with different minimum numbers of reads. I find no obvious difference if i remove only OTUs with 1 total read (commNosr.002) or OTUs with < 100 total reads (commNosr.100). So for now, I stick with removing all *singleton* OTUs (OTUs that appear in only one sample, regardless of number of reads in that OTU).

# To do
1. filter out low-read OTUs
2. alpha diversity
3. boral
4. mvabund
5. breakaway
6. rarefaction (chao, inext) # sample-based rarefaction

# Questions
1. Is MF "better" than individual forests (JC, EC, BB)?
2. Define "better":  
	+ More total species diversity/richness (sample-based rarefaction)
3. Can we predict composition with Landsat?
4. Do this with Kat's dataset too.
5. mvabund/boral
...
pool1 <- specpool(community_diptera, habitat_diptera$Habitat) %>% rownames_to_column("Habitat")
pool1
...
comm.fit.p3 <- boral(commNosrB, family = "poisson", num.lv = 3, row.eff = "fixed", calc.ics = FALSE, mcmc.control = mcmc.control)
summary(comm.fit.p3)
par(mfrow = c(2,2))
plot(comm.fit.p3) ## Plots used in residual analysis, 
#dev.off()
